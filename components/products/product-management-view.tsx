...
// Données de simulation pour les produits
const mockProducts = Array.from({ length: 30 }, (_, i) => ({
    id: `${i + 1}`,
    code: `ART-${1000 + i}`,
    name: `Article de Test ${String.fromCharCode(65 + (i % 26))}${i}`,
    family: ['Visserie', 'Peinture', 'Outillage', 'Sécurité'][i % 4],
    isActive: i % 12 !== 0,
    isPerishable: i % 10 === 0,
    isDiscountable: true,
    mainSupplier: "Fournisseur Principal SA"
}));

// Composants de vue pour les détails (placeholders)
const PricingInfoView = ({ product }) => (
    <div className="p-6">
        <h3 className="text-lg font-semibold mb-4 text-gray-900">Informations Prix</h3>
        <div className="space-y-4">
            <p className="text-gray-600">Détails des prix pour {product.name}</p>
        </div>
    </div>
);
const StockInfoView = ({ product }) => (
    <div className="p-6">
        <h3 className="text-lg font-semibold mb-4 text-gray-900">Informations Stock</h3>
         <p className="text-gray-600">Détails du stock pour {product.name}</p>
    </div>
);
const MovementHistoryView = ({ product }) => (
    <div className="p-6">
        <h3 className="text-lg font-semibold mb-4 text-gray-900">Historique Mouvements</h3>
         <p className="text-gray-600">Historique des mouvements de stock pour {product.name}</p>
    </div>
);
...
export function ProductManagementView() {
    ...
    return (
        <div className="h-screen bg-gray-50 flex">
            {/* Sidebar gauche pour les actions */}
            ...
            <div className="flex-1 bg-white">
                {isCreating ? (
                    <ProductForm initialData={null} onCancel={handleCancelCreation} />
                ) : selectedProduct ? (
                    <>
                        {activeView === 'profile' && <ProductForm initialData={selectedProduct} onCancel={() => {}} />}
                        {activeView === 'pricing' && <PricingInfoView product={selectedProduct} />}
                        {activeView === 'stock' && <StockInfoView product={selectedProduct} />}
                        {activeView === 'history' && <MovementHistoryView product={selectedProduct} />}
                    </>
                ) : (
                    <div className="h-full flex items-center justify-center">
                        <div className="text-center">
                            <ShoppingBasket className="mx-auto h-12 w-12 text-gray-400 mb-4" />
                            <h3 className="text-lg font-medium text-gray-900 mb-2">
                                Aucun article sélectionné
                            </h3>
                            <p className="text-gray-600">
                                Sélectionnez un article dans la liste pour voir ses détails
                            </p>
                        </div>
                    </div>
                )}
